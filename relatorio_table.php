...
class PDF extends FPDF
{
    ...
    public function gerarTabela($data)
    {
        ...
        $this->Cell(50, self::CELL_TABLE_H, $this->texto('Country'), 'BT', 0, 'C', true);
        $this->Cell(35, self::CELL_TABLE_H, $this->texto('Capital'), 'BT', 0, 'C', true);
        $this->Cell(30, self::CELL_TABLE_H, $this->texto('Area (sq km)'), 'BT', 0, 'C', true);
        $this->Cell(35, self::CELL_TABLE_H, $this->texto('Pop. (thousands)'), 'BT', 0, 'C', true);
        $this->Cell(50, self::CELL_TABLE_H, $this->texto('Country'), 'BT', 0, 'C', true);
        $this->Cell(35, self::CELL_TABLE_H, $this->texto('Capital'), 'BT', 0, 'C', true);
        $this->Cell(42, self::CELL_TABLE_H, $this->texto('Area (sq km)'), 'BT', 0, 'C', true);
        ...
        foreach($data as $row)
        {
            ...
            $this->Cell(35, self::CELL_TABLE_H, $this->texto($row[1]), '', 0,'C', $colorir_linha);
            $this->Cell(30, self::CELL_TABLE_H, number_format($row[2]), '', 0,'C', $colorir_linha);
            $this->Cell(35, self::CELL_TABLE_H, number_format($row[3]), '', 0,'C', $colorir_linha);
            $this->Cell(50, self::CELL_TABLE_H, $this->texto($row[4]), '', 0,'C', $colorir_linha);
            $this->Cell(35, self::CELL_TABLE_H, $this->texto($row[5]), '', 0,'C', $colorir_linha);
            $this->Cell(42, self::CELL_TABLE_H, number_format($row[6]), '', 0,'C', $colorir_linha);
...
$pdf->Cell(0, 10, $pdf->texto('Relatórios de Produtos'), 0, 1, 'C');
